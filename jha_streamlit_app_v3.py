...
page = st.sidebar.radio("Go to", ["Home (Overview)", "Search / Edit", "Download"])

# Home / Landing: styled rendering of first sheet
if page == "Home (Overview)":
    st.title("Overview")
    landing = data_dict[sheets[0]]
    # Try to render as markdown: if first column contains text rows, interpret as paragraphs
    if landing.shape[1] == 1:
        paragraphs = [str(x[0]) for x in landing.values.tolist() if pd.notna(x[0])]
        for p in paragraphs:
            # simple heuristic: lines starting with '#', '-' or numeric become headers/lists; else paragraph
            p = str(p)
            if p.strip().startswith("#"):
                st.markdown(p)
            elif p.strip().startswith("-") or p.strip().startswith("*"):
                st.markdown(p)
            else:
                st.markdown(p.replace("\\n", "  \n"))
    else:
        # If multi-column, render first few rows as formatted sections using first cell as header and second as content where possible
        for i in range(min(len(landing), 100)):
            ...
            if len(row) == 1:
                ...
                st.markdown(f"### {row[0]}")
                st.markdown(" ".join(row[1:]).replace("\\n", "  \n"))

# Search / Edit page (no charts)
    ...
    if sel_div == "-- Select Division --":
        ...
        hazard_text = ""
        # Try match by division column in hazards sheet
        hazard_div_col = find_col(hazards_sheet, ["division"])
        if hazard_div_col and hazard_col_haz and hazard_div_col in hazards_sheet.columns:
        ...
        # Fallback: show hazards column contents
        ...
        if control_div_col and control_col_ctrl and control_div_col in controls_sheet.columns:
